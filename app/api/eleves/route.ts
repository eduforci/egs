...
const matriculeNettoye = matricule
...
if (!rolesAutorises.includes(profile.role)) {
  return NextResponse.json(
    {
      error:
        "Vous n'êtes pas autorisé à créer un compte élève.",
    },
    { status: 403 }
  );
}
...
if (
  classeError ||
  !classe ||
  classe.etablissement_id !== profile.etablissement_id
) {
  return NextResponse.json(
    {
      error:
        "Classe invalide pour votre établissement.",
    },
    { status: 400 }
  );
}
...
const { data: anneeScolaire, error: anneeError } =
  await supabase
    .from("annees_scolaires")
    .select(
      "id, libelle, date_debut_inscriptions, date_debut, date_fin, active"
    )
    .eq(
      "etablissement_id",
...
// Les inscriptions peuvent commencer avant le début officiel
// des cours. Si date_debut_inscriptions est renseignée,
// elle devient donc la date de référence pour les inscriptions.
//
// Exemple :
// date_debut_inscriptions = 01/08/2026
// date_debut              = 14/09/2026
// date_fin                = 31/07/2027

const dateDebutInscriptions =
  anneeScolaire.date_debut_inscriptions ||
  anneeScolaire.date_debut;

if (
  aujourdHui < dateDebutInscriptions ||
  aujourdHui > anneeScolaire.date_fin
) {
  return NextResponse.json(
    {
...
let identifiant = aUnMatricule
  ? matriculeNettoye
  : genererIdentifiantProvisoire();
...
if (aUnMatricule) {
  ...
  // Sécurité contre une collision d'identifiant provisoire
...
const emailSynthetique =
  `eleve-${genererJetonEmail()}@eleves.egs.local`;
...
const { error: eleveError } =
  await admin.from("eleves").insert({
    id: userId,
    etablissement_id:
      profile.etablissement_id,
    classe_id,
    matricule: aUnMatricule
      ? matriculeNettoye
      : null,
    date_naissance:
      date_naissance || null,
...
// ---------------------------------------------------------
// 11. Création automatique de l'inscription
// ---------------------------------------------------------
...
if (inscriptionError) {
  ...
  return NextResponse.json(
    {
      error:
        `Impossible de créer l'inscription : ${inscriptionError.message}`,
    },
    { status: 500 }
  );
}
