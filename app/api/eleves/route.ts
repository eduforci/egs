  ...
  try {
    ...
    const { data: anneeScolaire, error: anneeError } =
      await supabase
        ...
        .select(
          "id, libelle, date_debut, date_fin, active"
        )
        .eq(
          "etablissement_id",
    ...
    // ---------------------------------------------------------
    // 5. Vérification de la période de l'année scolaire
    // ---------------------------------------------------------

    if (
      aujourdHui < anneeScolaire.date_debut ||
      aujourdHui > anneeScolaire.date_fin
    ) {
      return NextResponse.json(
        {
          error: `La date actuelle (${aujourdHui}) est en dehors de la période de l'année scolaire active ${anneeScolaire.libelle}.`,
        },
        { status: 400 }
      );
    }
