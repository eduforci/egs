...
export async function POST(req: NextRequest) {
  ...
  const { eleve_id, type, etablissement_partenaire, motif } = await req.json();

  if (!eleve_id || !type) {
    return NextResponse.json({ error: 'Élève et type sont obligatoires' }, { status: 400 });
  }
  if (!['sortant', 'entrant'].includes(type)) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      type,
      etablissement_partenaire: etablissement_partenaire || null,
      motif: motif || null,
      cree_par: user.id,
    })
    .select()
